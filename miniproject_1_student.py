...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    # Get list of words in the input sentence
    words = sentence.split(" ")
    count = 0
    
    # Iterate through each word and add its vector
    for word in words:
        embed = get_glove_embeddings(word, word_index_dict, embeddings, model_type)
        embedding += embed
        count += 1
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        for index, category in enumerate(categories):
            ...
            # Compute cosine similarity
            score = cosine_similarity(input_embedding, category_embedding)
            cosine_sim[index] = score
        ...
        # Get OpenAI embeddings for the input sentence
        input_embedding = get_openai_embeddings(st.session_state.text_search, model_name)
        
        # Ensure category embeddings are cached
        get_category_embeddings(embeddings_metadata)
        ...
        model_name = embeddings_metadata["model_name"]
        cache_key = "cat_embed_" + embeddings_metadata["embedding_model"] + "_" + model_name
        
        # Get Transformer embeddings for the input sentence
        input_embedding = get_sentence_transformer_embeddings(st.session_state.text_search, model_name)
    ...
    # Sort results
    sorted_cosine_similarity = sorted(
        cosine_sim.items(), 
